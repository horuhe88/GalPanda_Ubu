...
uint8_t WiFiClass::getEncrBySsid(int count, char * ssid)
{
	for (int i = 0; i < count; i++ ) {
		...
		if (0 == strcmp(ssid, _networkSsid[i])){
			printf("found ssid %s, encr=%d", ssid, getEncrType(_networkEncr[i]));
...
void WiFiClass::init()
{
	...
	fclose(fp);
	printf("wlan0 = %d, cmd is now %s", sizeof(ARDUINO_WLAN), cmd);
	if(0 == (strncmp(cmd, ARDUINO_WLAN, sizeof(ARDUINO_WLAN)-1))) {
		printf("found %s", ARDUINO_WLAN);
		...
		printf("ifconfig %s up", ARDUINO_WLAN);
		...
		printf("no %s found %s", ARDUINO_WLAN);
	}
}
...
int WiFiClass::begin(char* ssid)
{
	...
	if(_local_ip._sin.sin_addr.s_addr == 0) {
		printf("didnt get an IP address, cant connect");
		sprintf(cmd, "ifconfig %s down", ARDUINO_WLAN);
		system(cmd);
		_status = WL_IDLE_STATUS;
		return _status;
...
{
	char cmd[256];
	printf("begin ssid=%s,key_idx=%d,key=%s",ssid,key_idx,key);
	/*
...
int WiFiClass::begin(char* ssid, const char *passphrase)
{
	...
	printf("begin ssid=%s,passphrase=%s",ssid,passphrase);
	if(ssid == NULL)
		return WL_NO_SSID_AVAIL;
	if(passphrase == NULL)
		return WL_CONNECT_FAILED;
...
{
	_local_ip = local_ip;
	printf("config local ip %3d.%3d.%3d.%3d",
...
void WiFiClass::config(IPAddress local_ip, IPAddress dns_server)
{
	...
	printf("_dns_server ip %3d.%3d.%3d.%3d",
						...
						IPAddress gateway)
{
	...
	printf("_gateway ip %3d.%3d.%3d.%3d",
...
void WiFiClass::setDNS(IPAddress dns_server)
{
	...
	printf("setDNS _dns_server ip %3d.%3d.%3d.%3d",
		(_dns_server._sin.sin_addr.s_addr&0x000000FF),
		(_dns_server._sin.sin_addr.s_addr&0x0000FF00)>>8,
		(_dns_server._sin.sin_addr.s_addr&0x00FF0000)>>16,
		(_dns_server._sin.sin_addr.s_addr&0xFF000000)>>24	);
...
void WiFiClass::setDNS(IPAddress dns_server1, IPAddress dns_server2)
{
	...
	printf("setDNS _dns_server2 ip %3d.%3d.%3d.%3d",
...
uint8_t* WiFiClass::BSSID(uint8_t* bssid)
{
	...
	if (_status == WL_CONNECTED) {
		...
		printf("%s not connected", ARDUINO_WLAN);
		return NULL;
	}
}
...
uint8_t WiFiClass::encryptionType()
{
	...
	if (_status == WL_CONNECTED) {
		...
		printf("doing scan to get encryption");
		count = scanNetworks();
...
#ifdef __DEBUG
	printf("found %d networks in range", count);
	for(int i=0; i < count; i++) {
		printf("%d. ssid=%s, rssi=%d, , encr=%s",
